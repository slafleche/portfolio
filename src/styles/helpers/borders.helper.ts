...
import type {
  ...
  EdgeState,
  Corner,
} from './borders/borders.types';
import { edgeKeys } from './borders/borders.types';
...
const bordersFactory = (overwrites?: BorderDefaults) => {
  const defaults = {
    ...borderDefaults,
    overwrites
  };
  ...
  const fallbackWidth = (): string =>
    toCssOrIgnore(borderVars.width) ?? '0';
  const fallbackRadius = (): string =>
    toCssOrIgnore(borderVars.radius) ?? '0';
  ...
  // Final exported function with helpers
