...
- Must: Keep values in measurement space (MeasurementKit, color objects, etc.) until helpers emit CSS.
...
- Must: Do not coerce measurement values to primitive numbers/strings inside tokens; coercion only happens at adapter/emission boundaries using the sanctioned MeasurementKit APIs.
...
- Must: Avoid mixing raw CSS strings/numbers with MeasurementKit instances inside the same token object.
