/**
 * Helper for generating optimized Google Fonts CSS API URLs.
 *
 * Supports:
 *
 * - Per-font texts -> &text=... (omit/empty => no text param, just subset(s))
 * - Per-font weights -> "400" | ["400","700"] | ["100..900"]
 * - Italics -> builds :ital,wght@0,400;0,700;1,400;1,700 (sorted tuples)
...
/** De-dupe and sort weight tokens in ascending order */
...
function buildTextParam(
	...
	const raw = stripWhitespaceFromText ? joined.replace(/\s+/g, '') : joined;
...
export function generateGoogleFontUrls(
	...
	for (const [family, cfg] of Object.entries(fonts)) {
		...
		const textParam = buildTextParam(cfg.texts, stripWhitespaceFromText);
...
/** Convenience for SSR templates (string of <link> tags) */
...
/** Utility you can reuse in generators: collapse strings → unique-char string */
export function uniqueCharsFromStrings(
	input: string[],
	opts?: { stripWhitespace?: boolean },
): string {
	const joined = input.join('');
	const raw = opts?.stripWhitespace ? joined.replace(/\s+/g, '') : joined;
