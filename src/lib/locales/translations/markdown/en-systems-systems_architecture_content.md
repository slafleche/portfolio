...
piece of UI flows through a strict pipeline—tokens → helpers → modules →
...
unit safety through MeasurementKit (migrating soon to CSS Calipers). Modules
compose behavior, and styles are the only layer allowed to emit selectors,
enforced by custom ESLint rules and lint-staged guards.
