...
Calipers published on <span data-white-space="no-wrap">[element:NPMWordmark|en].</span>
That means unit mixups and invalid values get caught at authoring time, and the
output remains plain, debuggable CSS.
...
- svg optimization
- Site map
- locales (verify no duplicate IDs, verify no missing keys in other locale, parse MD files, you get actual JS error if location is missing or bad key instead of finding out in production)
- eslint/ts/prettier
