...
- [ ] Prettier core release (and its estree plugin) that formats this repo
      without known crashes on: - `app/layout.tsx` - `app/page.tsx` -
      `app/[LOCALE]/debug/formelements/*` -
      `tests/contact/ContactFormShell.test.tsx` -
      `tests/styles/colorWrap.helper.test.ts`
- [ ] A small, documented smoke-test script (for example, a
      `yarn     format:code:smoke` variant) that runs Prettier against a
      representative subset of the repo and fails loudly if the upgrade
      regresses formatting or reintroduces crashes.
