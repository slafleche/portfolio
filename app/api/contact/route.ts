...
type ResponseTelemetryOptions = Parameters<
...
const respondWithTelemetry = (
  ...
  const { startedAt, ipHash, logExtras, ...responseOptions } =
    options;
  const resolvedOk = responseOptions.okOverride ?? code === 'success';
  const httpStatus =
    responseOptions.status ?? (resolvedOk ? 200 : 400);
