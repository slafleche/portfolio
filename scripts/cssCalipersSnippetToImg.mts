...
const OUTPUT_NAME = `css-calipers-snippet${scaleSuffix}.png`;
...
const stylePath = path.join(
  ...
  'styles',
  'components',
  'mockCssCalipersSnippet.css.ts',
);
setFileScope(stylePath);
const { default: MockCssCalipersSnippet } = await import(
  '../src/components/MockCssCalipersSnippet'
);
endFileScope();

const markup = renderToStaticMarkup(
  React.createElement(MockCssCalipersSnippet, {
    ariaLabel: 'CSS-Calipers code example',
  }),
);
...
if (!box) {
  await browser.close();
  throw new Error('Unable to measure snippet size.');
}
