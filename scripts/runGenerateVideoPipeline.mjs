...
const confirmSync = async () => {
  ...
  console.log('Run cdn:sync --videos now? [Y/n]:');
...
if (args.includes('--help') || args.includes('-h')) {
  run('generate:videos:files', ['--help']);
  process.exit(0);
}

run('generate:videos:files', args);
run('generate:videos:posters', args);
...
if (runSync) {
  ...
  run('generate:videos:artifacts', args);
}
