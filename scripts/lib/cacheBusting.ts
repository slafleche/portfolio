...
export const hashBuffer = (buffer: Buffer | string, length: number) => {
...
export async function writeHashedFile({
	...
	const fileName = formatHashedFileName(logicalName, hash, ext, prefix);
	...
	const normalizedRoot = publicRoot.endsWith('/')
		...
		: publicRoot;
	const urlPath = `${normalizedRoot}/${fileName}`.replace(/\/+/g, '/');
	return {
		fileName,
		absolutePath,
		urlPath,
